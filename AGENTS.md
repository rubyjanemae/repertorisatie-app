...
Two OOREP repertories (both GPL v3) are shipped as static JSON, one file per chapter. Chapters are lazy-loaded and in-memory cached.

- **Repertorium Publicum** (English, 74k rubrics) in [public/repertory/](public/repertory/) (`mind.json`, `head.json`, …). These files contain hand edits that are not in OOREP; never regenerate them.
- **Kent, German translation** (68k rubrics) in [public/repertory/kent-de/](public/repertory/kent-de/) (`gemuet.json`, `kopf.json`, …). Generated by [scripts/build-kent-de.mjs](scripts/build-kent-de.mjs) from OOREP's `oorep.sql.gz`; rerun the script instead of editing these files.

A `chapterFile` with a folder prefix (`kent-de/gemuet`) points to Kent, without prefix (`mind`) to the Publicum; `repertoryOfChapterFile` in [repertoryData.ts](src/lib/repertoryData.ts) derives the source. Chapter lists per repertory live in `chaptersByRepertory` there. `searchRubrics` searches all repertories unless given a list, and ignores umlauts. Two indirection tables in `repertoryLookup.ts` translate Publicum sidebar chapter names to file names and path prefixes (e.g. "Generals" → `generalities.json`, prefix `Generalities, …`); add new chapter mappings there, not in the sidebar component.

OOREP's open dump contains only these two repertories. Its English Kent, Bönninghausen and others are login-only and not openly licensed; do not scrape them.
